...
class ZedNode(Node):
    ...
    def get_contours(self, binary_img):
        ...
        for idx, contour in enumerate(contours):
            ...
            # Filter contours based on aspect ratio and extent. MAY NEED TO TUNE
    ...
    def get_lane_pothole_images(self, img):
        ...
        try:
            ...
            # Resize each image
            width = int(lane_image.shape[1] * scale_percent / 100)
            height = int(lane_image.shape[0] * scale_percent / 100)
            dim = (width, height)

            lane_image_resized = cv2.resize(lane_image, dim, interpolation=cv2.INTER_AREA)
            img_resized = cv2.resize(img, dim, interpolation=cv2.INTER_AREA)
            pothole_image_resized = cv2.resize(pothole_image, dim, interpolation=cv2.INTER_AREA)

            # Stack the resized images horizontally
            combined_image = np.hstack([lane_image_resized, img_resized, pothole_image_resized])

            # Display the combined image
            cv2.imshow("Lanes, Original Image, Potholes", combined_image)
            cv2.waitKey(1)
